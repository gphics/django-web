...
function CircleListComponent({ circleRes }) {
  ...
  return <>
    {!!data?.length && data?.map((elem, i) => {
      ...
      return <Link key={i} href={"/circle/"+elem?.id} className="w-[130px] flex flex-col justify-center items-center h-[130px] bg-overlay rounded-sm m-1 hover:bg-accent-color transition-all duration-300"> 
        <MediaPresentationComponent media={elem?.media} name={elem?.name}  />
        <h4 className="text-[0.9em] mb-1"> <strong>{elem?.name.length <= 25 ? elem?.name : elem?.name.slice(0, 25) +".."}  </strong>  </h4>
