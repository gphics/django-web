...
function CircleListComponent({ circleRes }) {
  ...
  return <>
    {!!data?.length && data?.map((elem, i) => {
      ...
      return <Link key={i} href={"/circle/"+elem?.id} className="w-[150px] flex flex-col justify-center items-center p-2 h-[150px] bg-overlay m-1 hover:bg-accent-color transition-all duration-300"> 
