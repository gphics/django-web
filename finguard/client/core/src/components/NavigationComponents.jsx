...
function AuthNavComponent() {
    ...
    return <div>
        {/* bar click */}
        <section className="md:hidden my-1 z-10 pl-1">
        ...
        {/* all links */}
        <section id="nav-holder" className='flex justify-between items-center px-2 py-1 mb-2 max-md:absolute bg-white w-[100%] max-md:translate-x-[-100%] transition-transform duration-500 max-md:flex-col max-md:items-start shadow-xl'>
            <Link href="/">
            ...
            <section className="w-[65%] flex justify-around max-md:flex-col flex-auto">
                {linkArr.map((elem, index) => {
                    return <Link className="capitalize hover:poppins-bold px-2 py-1 hover:bg-accent-color" href={"/" + elem} key={index}> {elem} </Link>
                })}
            </section>
            <button onClick={logoutAction} type='button' className="px-3 py-1 bg-accent-color poppins-bold rounded-sm cursor-pointer hover:text-white">Logout</button>
        </section>
